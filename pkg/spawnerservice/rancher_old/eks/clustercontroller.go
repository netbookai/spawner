...
func AddNodeGroup(cluster *rnchrClient.Cluster, nodeSpawnRequest *pb.NodeSpawnRequest, tags map[string]string) (rnchrClient.ClusterSpec, error) {
	...
	newNodeGroup := rnchrClient.NodeGroup{
		...
		Labels: common.MapPtr(maps.SimpleReplaceMerge(
			map[string]string{constants.CreatorLabel: constants.SpawnerServiceLabel, constants.ProvisionerLabel: constants.RancherLabel, constants.NODE_NAME_LABEL: nodeSpawnRequest.NodeSpec.Name, constants.NODE_LABEL_SELECTOR_LABEL: nodeSpawnRequest.NodeSpec.Name, constants.INSTANCE_LABEL: nodeSpawnRequest.NodeSpec.Instance, "type": "nodegroup"},
			cluster.Labels,
		...
		ResourceTags: common.MapPtr(maps.SimpleReplaceMerge(
			map[string]string{constants.CreatorLabel: constants.SpawnerServiceLabel, constants.ProvisionerLabel: constants.RancherLabel, constants.NODE_NAME_LABEL: nodeSpawnRequest.NodeSpec.Name, constants.INSTANCE_LABEL: nodeSpawnRequest.NodeSpec.Instance},
			cluster.Labels,
			nodeSpawnRequest.NodeSpec.Labels,
		)),
...
func CreateCluster(awsCred rnchrClient.CloudCredential, clusterReq *pb.ClusterRequest, clusterName string, clusterTags map[string]string, nodeGroupTags map[string]string, subnets []string) *rnchrClient.Cluster {
	newNodeGroup := rnchrClient.NodeGroup{
		...
		MaxSize:              common.Int64Ptr(1),
		Gpu:                  common.BoolPtr(false),
		Labels:               common.MapPtr(maps.SimpleReplaceMerge(map[string]string{"node": clusterReq.Node.Name, constants.CreatorLabel: constants.SpawnerServiceLabel, constants.ProvisionerLabel: constants.RancherLabel}, clusterReq.Labels)),
		RequestSpotInstances: common.BoolPtr(false),
	...
	newCluster := rnchrClient.Cluster{
		...
		Labels:                  maps.SimpleReplaceMerge(map[string]string{constants.CreatorLabel: constants.SpawnerServiceLabel, constants.ProvisionerLabel: constants.RancherLabel}, clusterReq.Labels),
