...
func AddNodeGroup(cluster *rnchrClient.Cluster, nodeSpawnRequest *pb.NodeSpawnRequest, tags map[string]string) (rnchrClient.ClusterSpec, error) {
	...
	newNodeGroup := rnchrClient.NodeGroup{
		...
		Labels:               &map[string]string{"name": nodeSpawnRequest.NodeSpec.Name, "nodeLabelSelector": nodeSpawnRequest.NodeSpec.Name, "instance": nodeSpawnRequest.NodeSpec.Instance, "type": "nodegroup"},
		RequestSpotInstances: common.BoolPtr(false),
		ResourceTags:         &map[string]string{"name": nodeSpawnRequest.NodeSpec.Name, "instance": nodeSpawnRequest.NodeSpec.Instance},
