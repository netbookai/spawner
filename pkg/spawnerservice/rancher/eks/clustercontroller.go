...
func AddNodeGroup(cluster *rnchrClient.Cluster, nodeSpawnRequest *pb.NodeSpawnRequest, tags map[string]string) (rnchrClient.ClusterSpec, error) {
	...
	newNodeGroup := rnchrClient.NodeGroup{
		...
		Labels:               util.SimpleReplaceMerge(
			map[string]string{"creator": constants.SPAWNER_SERVICE_LABEL, "provisioner": constants.RANCHER_LABEL, "name": nodeSpawnRequest.NodeSpec.Name, "nodeLabelSelector": nodeSpawnRequest.NodeSpec.Name, "instance": nodeSpawnRequest.NodeSpec.Instance, "type": "nodegroup"},
			cluster.Labels,
		),
		RequestSpotInstances: common.BoolPtr(false),
		ResourceTags:         util.SimpleReplaceMerge(
			map[string]string{"creator": constants.SPAWNER_SERVICE_LABEL, "provisioner": constants.RANCHER_LABEL, "name": nodeSpawnRequest.NodeSpec.Name, "instance": nodeSpawnRequest.NodeSpec.Instance},
			cluster.Labels,
		),
...
func CreateCluster(awsCred rnchrClient.CloudCredential, clusterReq *pb.ClusterRequest, clusterName string, clusterTags map[string]string, nodeGroupTags map[string]string, subnets []string) *rnchrClient.Cluster {
	newNodeGroup := rnchrClient.NodeGroup{
		...
		MaxSize:              common.Int64Ptr(1),
		Gpu:                  common.BoolPtr(false),
		Labels:               util.SimpleReplaceMerge(map[string]string{"node": clusterReq.Node.Name, "creator": "spawner-service", "provisioner": "rancher"}, clusterReq.Labels),
		RequestSpotInstances: common.BoolPtr(false),
	...
	newCluster := rnchrClient.Cluster{
		...
		Labels:                  *util.SimpleReplaceMerge(map[string]string{"creator": constants.SPAWNER_SERVICE_LABEL, "provisioner": constants.RANCHER_LABEL}, clusterReq.Labels),
